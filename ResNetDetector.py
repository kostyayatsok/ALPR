...
class ResNetDetector:
    ...
    def detect_batch(self, TRAIN_PATH):
        if self.model is None:
            self.model = tf.keras.models.load_model('./buff_weights.h5')
        for batch in self.generator(TRAIN_PATH, aug=False):
            ...
            for image, points_true, points_pred in zip(batch[0], batch[1], points_preds):
                ...
                pts = np.hstack((points_pred[0:4].reshape(-1, 1), points_pred[4:8].reshape(-1, 1)))
                pts *= self.IMG_W
                plt.imshow(self.cropFromImage(image, pts))
                plt.show()
    def detect(self, image):
        if self.model is None:
            self.model = tf.keras.models.load_model('C:\\projects\\python\\nomeroff-net\\NomeroffNet\\buff_weights.h5')
        print(np.expand_dims(image, 0).shape)
        points_pred = self.model.predict(np.expand_dims(image, 0))[0]
        print(points_pred)
        plt.imshow(image)
        plt.scatter(points_pred[0] * self.IMG_W, points_pred[4] * self.IMG_W, s=50, marker='<', color='red')
        plt.scatter(points_pred[1] * self.IMG_W, points_pred[5] * self.IMG_W, s=50, marker='v', color='red')
...
# name = 'A 075 MP 102.jpg'
# img_path = VAL_PATH + name
